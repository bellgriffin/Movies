...
ggplot(movies, aes(log_country_count)) +
  geom_density()

ggplot(movies, aes(log_gross)) +
  geom_density()
...
ggplot(movies, aes(log(gross), color = rating)) +
  geom_density()
...
model <- lm(
  log_gross ~ log_budget + log_company_count + log_country_count + genre +
    rating + year + score + log_votes, movies
)
